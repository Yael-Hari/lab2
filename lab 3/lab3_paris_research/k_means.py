...
class KMeans:
    ...
    def run(self, points, random_seed):
        random.seed(random_seed)  # picking seed randomly
        # Randomly initiate clusters
        self._clusters = []
        initial_centroids = random.sample(points, self._k)  # list of randomly picked k points from existing stock of points
    ...
    def print_results(self):
        ...
        print('############################################')
        print('Average distance to centroid: {:f}. total loss: {:f}'.
              format(total_loss/total_points, total_loss))
        print('Sum of squared errors: {:f}. Mean squared error(MSE): {:f}'.
